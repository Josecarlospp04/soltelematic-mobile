...
Route::group(['middleware' => [], 'namespace' => 'Api\ClientLite', 'prefix' => 'clientlite'], function () {
    ...
    Route::group(['middleware' => ['auth:api', 'active_subscription']], function () {
        ...
        Route::get('sharing', ['uses' => 'SharingController@index']);
        Route::post('sharing', ['uses' => 'SharingController@store']);
        Route::delete('sharing/{id}', ['uses' => 'SharingController@destroy']);
        Route::get('pois/map', ['uses' => 'PoisController@map']);
        Route::get('routes/map', ['uses' => 'RoutesController@map']);
        
        // PARCHE SOLTELEMATIC MOBILE -- servicios de mantenimiento.
        Route::get('device/{device_id}/services', ['uses' => 'ServicesController@index']);
        Route::get('device/{device_id}/services/create', ['uses' => 'ServicesController@create']);
        Route::post('device/{device_id}/services', ['uses' => 'ServicesController@store']);
        Route::put('services/{service_id}', ['uses' => 'ServicesController@update']);
        Route::delete('services/{service_id}', ['uses' => 'ServicesController@destroy']);
